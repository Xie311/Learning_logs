...
typedef struct FDCAN_HANDLER {
    ...
    // 方法函数
    void (*FDCAN_Start)(void);              //启动
    void (*FDCAN_Rx_Filter_Init)(void);     //接收滤波器初始化
    void (*FDCAN_Interrupt_Init)(void);     //中断初始化
    uint8_t (*FDCAN_Send_MSG)(CAN_MSG *);   //发送消息
    void (*FDCAN_Update_RXFIFO_Status)(FDCAN_HandleTypeDef *, struct FDCAN_HANDLER *);  //更新_RXFIFO_状态
} FDCAN_HANDLER;
